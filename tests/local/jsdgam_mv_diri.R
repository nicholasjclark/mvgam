# Recovery fixture for the diri() multivariate Dirichlet family.
#
# Simulates K = 4 species at n_sites = 30 sites under a known
# rank-2 factor loading Z_true (column-centred to align with
# Stan's sum_to_zero_vector identification), softmax-transforms
# to category probabilities per site, then draws Dirichlet
# compositions at concentration phi_true = 30.
#
# Primary go/no-go:
#   cor(off_diag(true_cor), off_diag(post_cor)) > 0.7
#
# where the "true_cor" is cov2cor(Z_true %*% t(Z_true)) and the
# "post_cor" is the residual_cor(fit)$cor returned by mvgam's
# factor-loadings branch.
#
# Diagnostics reported:
#   - MAE on off-diagonals
#   - max|colSums(posterior_mean(Z))| (mode-1 health check; the
#     hard sum_to_zero_vector parameterisation pins this to ~0
#     by construction)
#   - divergent transitions + max-treedepth saturation
#   - min / max bulk ESS on Z entries
#
# Cached at /tmp/jsdgam_mv_diri_recovery_fit.rds.
...
# Column-centre Z_true so the true loadings live in the same
# identified subspace that the fit's sum_to_zero_vector
# parameterisation explores. cor() is shift-invariant via cov2cor
# but centring makes the truth-vs-posterior comparison sharper.
...
cache <- "/tmp/jsdgam_mv_diri_recovery_fit.rds"
if (file.exists(cache)) {
  cat("[cache] Loading diri recovery fit.\n")
  fit <- readRDS(cache)
} else {
  cat("[fit ] jsdgam(diri(), n_lv = 2, ", n_sites, " sites)\n", sep = "")
  fit <- jsdgam(
