# Recovery fixture for negative-binomial counts under a jsdgam.
#
# Simulates K = 5 species at n_sites = 60 sites with a known rank-2
# loading matrix Z_true. Each site draws lv ~ N(0, I), the species'
# log-means are mu_fixed + Z %*% lv, and counts are negative binomial
# at shape phi_true, so the species covary through the latent factors
# while carrying count over-dispersion of their own.
#
# Primary go/no-go:
#   cor(off_diag(true_cor), off_diag(post_cor)) > 0.6
#
# where true_cor = cov2cor(Z_true %*% t(Z_true)) and post_cor is
# residual_cor(fit)$cor. The threshold sits below the mvn value
# because a count likelihood carries less information per observation
# about the latent covariance than a Gaussian one does.
#
# Cached at tests/local/fixtures/val_mvgam_jsdgam_mv_nb.rds. Delete
...
cat("Simulated", n_sites, "sites x", K, "species. Counts range [",
    min(long_dat$y), ",", max(long_dat$y), "].",
...
cache <- local_fixture_path("val_mvgam_jsdgam_mv_nb.rds")
if (file.exists(cache)) {
  cat("[cache] Loading nb recovery fit.\n")
  fit <- readRDS(cache)
} else {
  cat("[fit ] jsdgam(negbinomial(), n_lv = 2, ", n_sites, " sites)\n",
      sep = "")
  fit <- jsdgam(
...
test_that("the residual correlation recovers the simulated one", {
  ...
  # species' latent column. The structural claims below separate the
  # two cases.
...
test_that("the shape parameter recovers the simulated dispersion", {
  # phi sets how far counts scatter around their mean. A phi off by
  # an order of magnitude leaves the latent covariance recovery
  # intact and makes every predictive interval the wrong width.
  ...
  expect_lt(abs(mean(phi_post) - phi_true) / phi_true, 0.75)
})
...
test_that("every prediction surface answers for every row", {
  ...
  # Counts: non-negative whole numbers, which a continuous draw
  # sampled from the wrong family would not be.
...
test_that("a shuffled newdata answers the same, in the new order", {
  # A prediction that places rows by position rather than by content
...
test_that("a newdata holding one species reads that species' state", {
  ...
  # levels the frame carries then numbers that species 1 whatever it
...
test_that("forecast is keyed by the species axis", {
  ...
  # A count family can be drawn from, so the response arm holds whole
  # numbers rather than the expectation under another name.
  fc <- forecast(fit, newdata = nd, ndraws = 20L, type = "response")
  drawn <- do.call(cbind, fc$forecasts)
  expect_true(all(drawn >= 0))
  expect_true(all(drawn == floor(drawn)))
})
...
test_that("marginaleffects reaches this fit and separates the species", {
  ...
  # A negative-binomial mean is strictly positive.
  ...
  # point of the grid is a draw being reported as the mean, and a
  # tolerance alone hides it wherever the mean sits near an integer.
  expect_false(all(pr$estimate == floor(pr$estimate)))

  # `predictions(type = "response")` reports the expected response,
  # which is what `posterior_epred()` returns.
