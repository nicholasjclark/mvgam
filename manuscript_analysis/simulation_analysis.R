...
pdf('Figures/FigS1_trenddynamics.pdf', width = 6.25, height = 5.85)
seed <- 500
set.seed(seed)
...
pdf('Figures/Fig1_extrapolation_example.pdf', width = 6.25, height = 5.85)
par(mfrow = c(2, 2),
    mgp = c(2.5, 1, 0),
...
drps_plot_dat$drps_upper <- log(as.numeric(drps_plot_dat$drps_upper))
drps_plot_dat$drps_lower <- log(as.numeric(drps_plot_dat$drps_lower))
drps_plot_dat$drps_med <- log(as.numeric(drps_plot_dat$drps_med))
ggplot(drps_plot_dat %>%
         dplyr::filter(trend_rel == 0.3),
       aes(y = (drps_upper + drps_med + drps_lower) / 3,
           x = model, fill = model)) +
  ylim(-1.25,0.5) +
  geom_boxplot() +
  facet_wrap(~prop_missing, labeller = as_labeller(prop_names)) +
  scale_fill_viridis(discrete = T, begin = 0.2, end = 1, guide = FALSE) +
  theme_bw() + coord_flip() + labs(x = '', y = '',
                                   title = 'Moderate dynamics') +
...
ggplot(drps_plot_dat %>%
  ...
  facet_wrap(~prop_missing, labeller = as_labeller(prop_names)) +
  scale_fill_viridis(discrete = T, begin = 0.2, end = 1, guide = FALSE) +
  theme_bw() + coord_flip() + labs(x = '', y = 'log(DRPS) calibration (lower is better)',
                                   title = 'Strong dynamics') +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank()) -> plot2

pdf('Figures/Fig2_simulation_drps_missing_plot.pdf')
...
pdf('Figures/FigS2_simulation_drps_nseries_plot.pdf')
cowplot::plot_grid(plot1, plot2, ncol = 1)
dev.off()
...
pdf('Figures/FigS3_simulation_coverage_missing_plot.pdf')
cowplot::plot_grid(plot1, plot2, ncol = 1)
dev.off()
...
ggplot(coverage_plot_dat %>%
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        panel.grid.minor = element_blank(),
        panel.background = element_blank()) -> plot2
pdf('Figures/Fig3_simulation_coverage_nseries_plot.pdf')
cowplot::plot_grid(plot1, plot2, ncol = 1)
dev.off()
