...
pdf('Figures/Fig5_ixodes_example.pdf', width = 6.25, height = 5.85)
par(mfrow = c(2, 2),
    mgp = c(2.5, 1, 0),
    mai = c(0.6, 0.6, 0.2, 0.2))
plot_mvgam_smooth(fit_hyp3$out_gam_mod, series = 3, smooth = 'season')
plot_mvgam_smooth(fit_hyp3$out_gam_mod, series = 3, smooth = 'cum_gdd')
plot_mvgam_fc(fit_hyp3$out_gam_mod, series = 3,
                  data_test = all_data$data_test,
              hide_xlabels = TRUE)
axis(1, at = seq(0, dim(fit_hyp3$out_gam_mod$ytimes)[1],
                 b = 52), labels = seq(2015, 2020), cex.axis = 1)
text(x=NROW(all_data$data_train) / NCOL(fit_hyp3$out_gam_mod$ytimes) + 9,
     y=12, labels = 'Forecast horizon', srt = -90)
plot_mvgam_trend(fit_hyp3$out_gam_mod, series = 3,
              data_test = all_data$data_test,
              hide_xlabels = TRUE)
axis(1, at = seq(0, dim(fit_hyp3$out_gam_mod$ytimes)[1],
                 b = 52), labels = seq(2015, 2020), cex.axis = 1)
...
pdf('Figures/FigS8_ixodes_realisations.pdf', width = 6.25, height = 5.85)
par(mfrow = c(2, 2),
    mgp = c(2.5, 1, 0),
    mai = c(0.6, 0.6, 0.2, 0.2))
plot_mvgam_smooth(fit_hyp3$out_gam_mod, series = 3, smooth = 'season',
...
pdf('Figures/FigS7_ixodes_ppcs.pdf', width = 6.25, height = 5.85)
par(mfrow = c(2, 2),
    mgp = c(2.5, 1, 0),
    mai = c(0.6, 0.6, 0.2, 0.2))
ppc(fit_hyp3$out_gam_mod, series = 3, type = 'cdf')
mtext('Training period (retrodictive) checks', side=3, line=0.2, at=19)
ppc(fit_hyp3$out_gam_mod, series = 3, type = 'mean')

ppc(fit_hyp3$out_gam_mod, series = 3, type = 'cdf',
               data_test = all_data$data_test)
mtext('Testing period (predictive) checks', side=3, line=0.2, at=8)
ppc(fit_hyp3$out_gam_mod, series = 3, type = 'mean',
               data_test = all_data$data_test)
dev.off()
...
load('Results/amb_models.rda')
all_data <- prep_neon_data(species = 'Ambloyomma_americanum', split_prop = 0.8)
pdf('Figures/Fig6_amb_seasonalities.pdf', width = 6.25, height = 5.85)
par(mfrow = c(2, 2),
    mgp = c(2.5, 1, 0),
    mai = c(0.6, 0.6, 0.2, 0.2))
plot_seasonality(1)
...
pdf('Figures/Fig7_amb_uncertainties.pdf', width = 6.25, height = 5.85)
par(mfrow = c(2, 2),
    mgp = c(2.5, 1, 0),
    mai = c(0.6, 0.6, 0.2, 0.2))
for(i in c(1, 7, 9, 16)){
...
pdf('Figures/FigS9_amb_res.pdf', width = 6.25, height = 5)
