...
calculate_pit = function(out_gam_mod, series, data_test, data_train){
  ...
  if(out_gam_mod$fit_engine == 'stan'){

    # For stan objects, ypred is stored as a vector in column-major order
    preds <- MCMCvis::MCMCchains(out_gam_mod$model_output, 'ypred')[,seq(series,
                                                                         dim(MCMCvis::MCMCchains(out_gam_mod$model_output, 'ypred'))[2],
                                                                         by = NCOL(out_gam_mod$ytimes))]
  } else {
