...
```
These functions are showing the marginal contribution to the seasonal smooth function to the linear predictor (on the `log` scale), so they are clearly allowed to move into ridiculous spaces that we should give *very* little prior plausibility to:
```{r}
exp(-25)
exp(25)
```

What happens if we set `k` to a smaller value, resulting in less flexibility?
```{r, message=FALSE, warning=FALSE}
lynx_mvgam_prior <- mvgam(data_train = lynx_train,
               data_test = lynx_test,
...
We can now condition the model on observed data in `Stan` using MCMC sampling with the `Cmdstan` interface (installation links for `rstan` and `cmdstanr` are found [here](https://mc-stan.org/users/interfaces/rstan) and [here](https://mc-stan.org/cmdstanr/articles/cmdstanr.html)).
```{r, message=FALSE, warning=FALSE}
