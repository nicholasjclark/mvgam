      ...
      if(smooths_included){
        inits <- function() {
          list(b_raw = runif(model_data$num_basis, -2, 2),
               sigma_obs = runif(NCOL(model_data$ytimes), 0.5, 3),
  ...
  if(family == 'lognormal'){
    if(trend_model %in% c('None', 'GP')){
      if(smooths_included){
        inits <- function() {
          list(b_raw = runif(model_data$num_basis, -2, 2),
               sigma_obs = runif(NCOL(model_data$ytimes), 0.2, 2),
               lambda = runif(model_data$n_sp, 5, 25))
        }
      } else {
        inits <- function() {
    ...
    if(trend_model %in% c('None', 'GP')){
      if(smooths_included){
        inits <- function() {
          list(b_raw = runif(model_data$num_basis, -2, 2),
               sigma_obs = runif(NCOL(model_data$ytimes), 0.5, 3),
               nu = runif(NCOL(model_data$ytimes), 1, 5),
               lambda = runif(model_data$n_sp, 5, 25))
        }
      } else {
        inits <- function() {
  ...
  if(family == 'beta'){
    if(trend_model %in% c('None', 'GP')){
      if(smooths_included){
        inits <- function() {
          list(b_raw = runif(model_data$num_basis, -2, 2),
               phi = runif(NCOL(model_data$ytimes), 0.1, 10),
               lambda = runif(model_data$n_sp, 5, 25))
        }
      } else {
        inits <- function() {
