...
plot.mvgam_irf = function(x, series = 1, ...){
  ...
  responses <- do.call(rbind, lapply(seq_along(impulse_responses), function(j){
    data.frame(horizon = 1:h,
               ...
               resp_var = sort(rep(paste0('Process~', 1:n_processes),
                                   NROW(impulse_responses[[j]][[1]]))))
  })) %>%
    dplyr::mutate(resp_var = paste0('Process~',
                                    series,
                                    ' %->% ',
                                    resp_var)) %>%
    ...
    dplyr::summarise(med = median(imp_resp),
  ...
  # Plot the IRFs
  ggplot2::ggplot(data = responses,
                  ggplot2::aes(x = horizon,
                               y = med)) +
    ...
    ggplot2::geom_ribbon(mapping = ggplot2::aes(ymin = lower4,
                                                ymax = upper4),
                         fill = "#A25050") +
    ggplot2::geom_line(col = "#8F2727",
                       linewidth = 1) +
    ggplot2::geom_hline(yintercept = 0,
                        linetype = "dashed",
                        colour = "black") +
    ggplot2::facet_wrap(~resp_var,
                        scales = 'free_y',
                        labeller = ggplot2::label_parsed) +
    ggplot2::labs(x = "Horizon",
                  y = paste0(attr(x, 'irf_type'),
                             " impulse response")) +
    ggplot2::theme_bw()
}
