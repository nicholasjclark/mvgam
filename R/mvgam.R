...
mvgam = function(formula,
  ...
  # Function to vectorise a stan model's likelihood for quicker computation
  vectorise_lik = function(model_file, model_data, family = 'Poisson',
                           threads = 1){
    if(threads > 1){
      if(family == 'Poisson'){
        if(any(grepl('functions {', model_file, fixed = TRUE))){
          model_file[grep('Stan model code', model_file)] <-
            paste0('functions {\n',
                   'real partial_sum(real[] mu_slice,\n',
                   'int start, int end,\n',
                   'int[] y) {\n',
                   'return poisson_log_lpmf(y[start:end] | mu_slice);\n',
                   '}\n')
        } else {
          model_file[grep('Stan model code', model_file)] <-
        ...
        if(any(grepl('functions {', model_file, fixed = TRUE))){
          model_file[grep('Stan model code', model_file)] <-
            paste0('functions {\n',
                   ...
                   'int start, int end,\n',
                   'int[] y, real[] phi) {\n',
                   'return neg_binomial_2_lpmf(y[start:end] | mu_slice, phi[start:end]);\n',
                   '}\n')
        } else {
          model_file[grep('Stan model code', model_file)] <-
            paste0('//Stan model code generated by package mvgam\n',
    ...
    if(family == 'Poisson'){
      if(threads > 1){
        model_file[lik_line] <- paste0('// likelihood functions\n',
                                           'real flat_mus[n_nonmissing];\n',
                                           'flat_mus = to_array_1d(to_vector(mus)[obs_ind]);\n',
    ...
    if(family == 'Negative Binomial'){
      if(threads > 1){
        model_file[lik_line] <- paste0('// likelihood functions\n',
                                       ...
                                       'flat_mus = exp(to_array_1d(to_vector(mus)[obs_ind]));\n',
                                       'flat_rs = inv(to_array_1d(rep_each(r_inv, n)[obs_ind]));\n',
                                       'target += reduce_sum(partial_sum,\n',
                                       'flat_mus,\n',
                                       'grainsize,\n',
                                       'flat_ys,\n',
        ...
        model_file[lik_line] <- paste0('// likelihood functions\n',
                                       ...
                                       'flat_mus = to_array_1d(to_vector(mus)[obs_ind]);\n',
                                       'flat_ys ~ neg_binomial_2(exp(flat_mus), inv(flat_rs));\n}\n')
      }
    ...
    if(threads > 1){
      model_file[obs_line] <- paste0('int<lower=0, upper=1> y_observed[n, n_series];',
                                     '// indices of missing vs observed\n',
                                     ...
                                     'int<lower=-1> y[n, n_series];',
                                     '// time-ordered observations, with -1 indicating missing\n',
  ...
  if(!run_model){
    ...
    if(use_stan){
      ...
      if(offset){
        stan_objects$stan_file[grep('eta = to_vector(b * X);',
                                    stan_objects$stan_file, fixed = TRUE)] <-
          'eta = to_vector(b * X) + offset;'
      }

      vectorised <- vectorise_lik(model_file = stan_objects$stan_file,
