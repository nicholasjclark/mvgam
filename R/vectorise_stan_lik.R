...
vectorise_stan_lik = function(model_file, model_data, family = 'Poisson',
  ...
  if(trend_model == 'RW'){
    if(any(grepl('// dynamic factor estimates', model_file, fixed = TRUE))){
      init_trend_line <- grep('LV[1, j] ~ normal(0, 0.5)',
                              model_file, fixed = TRUE) - 1
      model_file <- model_file[-c(init_trend_line:(init_trend_line + 2))]
      model_file[init_trend_line] <-
        'LV[1, 1:n_lv] ~ normal(0, 0.5);'

      remainder_line <- grep('LV[2:n, j] ~ normal(LV[1:(n - 1), j], 0.5)',
                             model_file, fixed = TRUE) - 1
  ...
  if(trend_model == 'AR1'){
    if(any(grepl('// dynamic factor estimates', model_file, fixed = TRUE))){
      ...
      remainder_line <- grep('LV[2:n, j] ~ normal(ar1[j] * LV[1:(n - 1), j], 0.5)',
                             model_file, fixed = TRUE) - 1
      model_file <- model_file[-c(remainder_line:(remainder_line + 2))]
      model_file[remainder_line] <-
        paste0('for(j in 1:n_lv){\n',
               'LV[2:n, j] ~ normal(ar1[j] * LV[1:(n - 1), j], 0.5);\n',
               '}')
      model_file = readLines(textConnection(model_file), n = -1)
    } else {
      init_trend_line <- grep('trend[1, s] ~ normal(0, sigma[s])',
  ...
  if(trend_model == 'AR2'){
    if(any(grepl('// dynamic factor estimates', model_file, fixed = TRUE))){
      ...
      second_line <- grep('LV[2, j] ~ normal(LV[1, j] * ar1[j], 0.5)',
                          model_file, fixed = TRUE) - 1
      model_file <- model_file[-c(second_line:(second_line + 2))]
      model_file[second_line] <-
        'LV[2, 1:n_lv] ~ normal(LV[1, 1:n_lv] * ar1, 0.5);'
      ...
      model_file[remainder_line] <-
        paste0('for(j in 1:n_lv){\n',
               'LV[3:n, j] ~ normal(ar1[j] * LV[2:(n - 1), j] + ar2[j] * LV[1:(n - 2), j], 0.5);\n',
               '}')
      model_file = readLines(textConnection(model_file), n = -1)
    } else {
      init_trend_line <- grep('trend[1, s] ~ normal(0, sigma[s])',
  ...
  if(trend_model == 'AR3'){
    if(any(grepl('// dynamic factor estimates', model_file, fixed = TRUE))){
      ...
      model_file[third_line] <-
        'LV[3, 1:n_lv] ~ normal(LV[2, 1:n_lv] * ar1 + LV[1, 1:n_lv] * ar2, 0.5);'
      ...
      model_file[remainder_line] <-
        paste0('for(j in 1:n_lv){\n',
               'LV[4:n, j] ~ normal(ar1[j] * LV[3:(n - 1), j] + ar2[j] * LV[2:(n - 2), j] + ar3[j] * LV[1:(n - 3), j], 0.5);\n',
               '}')
      model_file = readLines(textConnection(model_file), n = -1)
    } else {
      init_trend_line <- grep('trend[1, s] ~ normal(0, sigma[s])',
