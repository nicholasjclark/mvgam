...
## ---------------------------------------------------------------------------------------------
summary(glm(y ~ series + time, data = simdat$data_train, family = poisson()))
...
data.frame(series = simdat$data_train$series, time = simdat$data_train$time) %>%
...
bad_times %>%
  dplyr::right_join(expand.grid(
    time = seq(min(bad_times$time), max(bad_times$time)),
...
bad_levels <- data.frame(
  time = 1:8,
  series = factor('series_1', levels = c('series_1', 'series_2')),
  outcome = rnorm(8)
)
...
## ----error = TRUE-----------------------------------------------------------------------------
try({
  get_mvgam_priors(outcome ~ 1, data = good_levels, family = gaussian())
})
...
## ----error = TRUE-----------------------------------------------------------------------------
try({
  get_mvgam_priors(outcome ~ cov, data = miss_dat, family = gaussian())
})
...
plot_mvgam_series(data = simdat$data_train, y = 'y', series = 'all')


## ----fig.alt = "Plotting time series features for GAM models in mvgam"------------------------
plot_mvgam_series(data = simdat$data_train, y = 'y', series = 1)
...
## ---------------------------------------------------------------------------------------------
model_dat %>%
  dplyr::mutate(series = plotID, y = target) %>%
  dplyr::mutate(siteID = factor(siteID), series = factor(series)) %>%
...
## ----error=TRUE-------------------------------------------------------------------------------
try({
  get_mvgam_priors(y ~ 1, data = model_dat, family = poisson())
})
