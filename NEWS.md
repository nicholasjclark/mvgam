...
* `stability()` now returns the posterior median and interval of each metric, matching `irf()`, `fevd()` and the other post-fit surfaces. Pass `summary = FALSE` for the per-draw metrics, which is what `plot()` draws histograms of. It also gained `ndraws` and `draw_ids`; each draw costs a Lyapunov solve, so a wide panel answers roughly four times faster from a subset. The summary carries each metric's binned posterior, so `plot()` draws the same histogram it always did without the draws behind it: a stability metric is usually read for whether its mass crosses zero, which a median and an interval alone do not answer
