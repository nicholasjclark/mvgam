...
- [ ] 2.0 Implement Core Prior and Stan Generation Systems
  ...
  - [ ] 2.6 Implement prior flow to trend stanvar generators
    ...
    - [ ] 2.6.15 Create map_trend_priors() function - extract relevant priors from brmsprior, handle ar{lag}_trend patterns - NEXT: Function should leverage existing get_trend_prior_spec() from registry system for extensibility (no hardcoded trend types). Takes brmsprior object + trend_type, uses extract_prior_string() helper for parameter matching, returns named list of Stan distribution strings ready for stanvar generators.
