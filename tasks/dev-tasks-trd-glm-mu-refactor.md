...
- [ ] **6.0 Complete Hierarchical System with Incremental Validation**
  ...
  - [ ] 6.7 **Eliminate VAR Competing Pathway & Complete AR/ZMVN**: Use pathfinder agent to locate VAR's competing hierarchical system (lines 3770-3805 in stan_assembly.R) that bypasses shared `add_hierarchical_support()`. Remove VAR-specific parameter generation while preserving coefficient matrix transformation logic. Complete AR/ZMVN shared system usage by implementing proper group-specific covariance application to innovations (missing from current partial implementation). Use code-reviewer agent to ensure proper integration. Regenerate all 3 models and have stan-code-expert verify unified approach. Task complete when VAR uses shared system and AR/ZMVN have complete implementations.
