  ...
  - [x] **2.3 prepare_predictions.mock_stanfit() Implementation**
    ...
    - [ ] 2.3.5 Implement helper function `extract_linpred_from_prep(prep, resp = NULL)` in `R/predictions.R`. **Implementation Strategy**: Pure R, fully vectorized (no loops over draws). Follow brms pattern: matrix ops via BLAS for all draws simultaneously. Break into reviewable chunks.
      ...
      - [ ] 2.3.5.4 **Special terms support** (smooths, random effects, GPs): Extend `extract_linpred_from_prep()` to handle all brms special terms beyond fixed effects. Break into exploratory and implementation phases.
        ...
        - [ ] 2.3.5.4.2.5 **PRIORITY: Investigate nested and correlated random effects in mvgam**: Before implementing RE prediction support, fully investigate how mvgam generates Stan code and data for complex random effects patterns. Test nested RE `(1 | site/plot)`, correlated RE `(x | group)`, crossed RE `(1 | site) + (1 | observer)`, and their interaction with different trend formulas. Document working combinations, identify Stan compilation issues, and ensure stancode/standata generation is robust. Create comprehensive test cases for all supported RE patterns in both observation and trend models. This investigation is required to ensure our prediction implementation handles all valid mvgam RE patterns correctly.
