  ...
  - [x] **2.3 prepare_predictions.mock_stanfit() Implementation**
    ...
    - [ ] 2.3.5 Implement helper function `extract_linpred_from_prep(prep, resp = NULL)` in `R/predictions.R`. **Implementation Strategy**: Pure R, fully vectorized (no loops over draws). Follow brms pattern: matrix ops via BLAS for all draws simultaneously. Break into reviewable chunks.
      ...
      - [ ] 2.3.5.4 **Special terms support** (smooths, random effects, GPs): Extend `extract_linpred_from_prep()` to handle all brms special terms beyond fixed effects. Break into exploratory and implementation phases.
        - [ ] 2.3.5.4.1 **Exploration**: Create `tasks/explore_special_terms.R` using `devtools::load_all()`. Load fit3 (smooths + VAR) and any other models with special terms. Generate prep objects and examine `prep$sdata` structure. Document: (1) How smooth terms appear (Zs_*, Xs_* matrices and corresponding parameters like s_*, sds_*), (2) How random effects appear (Z_*, J_* matrices and r_* parameters), (3) How GP terms appear (if present), (4) Parameter naming conventions for each type. Save findings to understand what needs to be implemented.
        - [ ] 2.3.5.4.2 **Smooth terms implementation**: In `extract_linpred_univariate()` and `extract_linpred_multivariate()`, add smooth term support. Identify smooth basis matrices from prep$sdata (Zs_* patterns for univariate, Zs_resp_* for multivariate). Extract smooth parameters (s_* or zs_* patterns). Implement vectorized computation: sum over all smooths of `s_draws %*% t(Zs)`. Handle multiple smooths per response. Verify dimensions match between basis matrices and parameters.
        ...
        - [ ] 2.3.5.4.5 **Testing**: Create `tasks/test_special_terms.R` with comprehensive tests. Test fit3 (smooths) to verify smooth term contributions are correctly computed. Compare total linear predictor (fixed + smooth + RE) against expected values. Verify dimensions, no NAs, and sensible magnitudes. Test both univariate and multivariate models with special terms.
