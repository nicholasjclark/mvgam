...
text(x=NROW(all_data$data_train) / NCOL(fit_hyp3$out_gam_mod$ytimes) + 6.5,
     y=20, labels = 'Forecast horizon', srt = -90)
...
plot_seasonality = function(i){
  ...
  ylab <- paste0('s(season) for ', levels(fit_hyp2$out_gam_mod$obs_data$series)[i])
  plot(1, type = "n",
       xlab = 'season',
       ylab = ylab,
...
pdf('NEON_manuscript/Figures/FigS8_amb_res.pdf', width = 6.25, height = 5.85)
plot(fit_hyp2$out_gam_mod, type = 're')
dev.off()
