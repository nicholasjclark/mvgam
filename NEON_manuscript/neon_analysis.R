...
plot_mvgam_smooth(fit_hyp3$out_gam_mod, series = 4, smooth = 'season')
plot_mvgam_smooth(fit_hyp3$out_gam_mod, series = 4, smooth = 'cum_gdd')
plot_mvgam_fc(fit_hyp3$out_gam_mod, series = 4,
...
plot_mvgam_trend(fit_hyp3$out_gam_mod, series = 4,
              data_test = all_data$data_test,
              hide_xlabels = TRUE)
axis(1, at = seq(0, dim(fit_hyp3$out_gam_mod$ytimes)[1],
                 b = 26), labels = seq(2015, 2020), cex.axis = 1)
...
pdf('NEON_manuscript/Figures/FigS7_amb_res.pdf', width = 6.25, height = 5.85)
