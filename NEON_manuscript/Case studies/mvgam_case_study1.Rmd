...
Gather the data into a dataframe format and calculate observation lags for lag = 1, 2, and 3; Also create a randomly distributed covariate to ensure we can have at least one smooth term in the model (at present it is necessary to have at least one smooth term to fit `mvgam` models, but this restriction will be eased in future versions). Keep the last `50` observations to make sure there are no `NAs` in the data
```{r}
...
Draw 1000 parameter estimates from the GAM posterior and use these to propaagate the forecast for 10 timesteps ahead
```{r}
...
```{r}
mod_skeleton <- mvgam(y ~ s(cov1, k = 3),
                 data_train = data_train,
                 data_test = data_test,
                 family = 'nb',
                 trend_model = 'AR3',
...
The same steps above can be used to generate `Stan` data and modelling files, with all options available apart from the use of latent variable trends (these are currently being implemented in the development version of `mvgam`):
```{r}
mod_skeleton <- mvgam(y ~ s(cov1, k = 3),
                 data_train = data_train,
                 data_test = data_test,
...
Why does autocorrelated observation model perform so badly? Not because of the smooth function estimates, which both estimate similar functions for the fake covariate
```{r, fig.width = 6, fig.height = 5, fig.align='center'}
plot(m_mgcv, shade = T)
```
...
Q-Q plots give a bit more insight, as they clearly indicate the dynamic latent trend model shows a better fit of the observation process to the data compared to the autocorrelated observation model
```{r, fig.width = 6, fig.height = 5, fig.align='center'}
...
plot(trend[4:53], xlim = c(0, 50), type = 'l', lwd = 2, col = 'grey',
     ylab = 'True trend')
plot(m_mvgam, 'trend', data_test = data_test)
layout(1)
```
...
plot(trend[4:53], xlim = c(0, 50), type = 'l', lwd = 2, col = 'grey',
     ylab = 'True trend')
plot(m_mvgam, 'trend', data_test = data_test, realisations = TRUE)
layout(1)
```
...
This version (with logged lags) performs much better than the raw lag version, with sensible forecasts that are not blowing up to infinity. However the forecast is still inferior to the dynamic GAM where the latent temporal process is estimated separately from the observation process, at least for this particular simulation. There are of course many other advantages to the state-space representation that is used by `mvgam`, including: 
  
1. It is far easier to handle missing values (which will result in loss of many observations when fitting `mgcv` models, especially at higher order lags) and measurement error  
  
2. Latent trend processes provide recursive expressions for h-step ahead prediction and historical filtering  
  
3. Irregularly sampled observational time series can be readily modelled as the latent temporal process can simply be 'forecasted' over the missing observations  
...
with(model_dat, plot(time, y, 'l', lwd = 2, ylab = 'Ling cod landings'))
```

And for the covariate (whiting landings)
```{r, fig.width = 5, fig.height = 4, fig.align='center'}
with(model_dat, plot(time, p_whiting, 'l', lwd = 2, ylab = 'Whiting landings'))
```
...
```{r, fig.width = 5, fig.height = 4, fig.align='center'}
plot(m_mvgam2, series = 1, type = 'forecast', data_test = data_test)
```

```{r, fig.width = 5, fig.height = 4, fig.align='center'}
plot(m_mvgam, series = 1, type = 'forecast', data_test = data_test)
```
...
```{r, fig.width = 5, fig.height = 4, fig.align='center'}
plot_mvgam_smooth(m_mvgam2, series = 1, smooth = 'fake_cov', residuals = T)
```
...
pfilter_fc <- pfilter_mvgam_fc(file_path = "pfilter",
                       n_cores = 3, data_test = model_dat[31:36,],
