...
```{r, message=FALSE, warning=FALSE}
mod3 <- mvjagam(formula =  y ~ te(mintemp, lag, k = c(8, 4)) +
                 te(precip, lag, k = c(8, 4)),
                data_train = data_train,
                data_test = data_test,
                family = 'tw',
...
```{r, fig.width = 5, fig.height = 4, fig.align='center'}
plot(log(MCMCvis::MCMCchains(mod3$jags_output, 'twdis')),
     log(MCMCvis::MCMCchains(mod3$jags_output, 'tau')),
...
The version with the latent trend reduces in-sample DIC and will provide more realistic forecasts
```{r}
dic(mod2)
dic(mod3)
```
...
```{r, fig.width = 5, fig.height = 4, fig.align='center'}
plot_mvgam_fc(mod3, series = 1, data_test = data_test, ylim = c(0, 100))
```
...
preds <- predict(mod3, series = 1, newdata = newdata, type = 'link')
offset <- mean(preds)
...
for(i in 1:6){
  ...
  preds <- predict(mod3, series = 1, newdata = newdata, type = 'link') - offset
...
```{r, fig.width = 5, fig.height = 4, fig.align='center'}
ppc(mod3, data_test = data_test, series = 1, type = 'cdf')
```
