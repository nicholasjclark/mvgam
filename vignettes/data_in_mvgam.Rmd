...
## Example with simulated bird-song counts

To give one example of how a multi-series count dataset is laid out for
`mvgam` modelling, we use the stock `birdsong` data. This is a
simulated weekly count series for four bird species observed over 80
weeks. The truth combines a shared cyclic seasonal cycle on
week-within-year, species-specific random intercepts on the log scale,
and a four-series autoregressive process with cross-species
innovation correlation (two species rise together, two move opposite).
Datasets of this shape (overdispersed counts, strong seasonality, and
correlated trajectories across series) are common in ecological
monitoring and exercise most of `mvgam`'s observation and trend
machinery in a single fit.
...
The dataset already includes the columns that `mvgam` expects:
`series` (factor identifying each panel) and `time` (integer week
index), alongside the response `y` (weekly count) and the
covariate `week_in_year` for the seasonal smooth. The other columns
(`species`, `week`, `count`) are aliases kept for readability.

A quick sanity check via `get_mvgam_priors()`:
```{r error=TRUE}
get_mvgam_priors(
  y ~ s(week_in_year, bs = "cc", k = 8) +
    s(species, bs = "re"),
...
This confirms the observation formula, trend formula and family are
all compatible with the data. The trend model is specified as an
expression inside `trend_formula`; `AR(p = 1, cor = TRUE)` adds a
multivariate AR(1) latent process with a correlated innovation
covariance across the four species.

We can also construct (without fitting) the full Stan model with
`mvgam(..., run_model = FALSE)`. Using the `cmdstanr` backend gives
auto-formatted Stan code that is easier to inspect for any
inefficiencies before launching long sampling runs:
...
The resulting object contains the model code and data ready for
sampling:
```{r}
