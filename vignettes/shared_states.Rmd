...
fake_mod <- mvgam(
  ...
  # process model formula: shared seasonal smooth plus AR1
  # dynamics. Each latent process model shares the SAME smooth
  # but has DIFFERENT autoregressive dynamics; processes are
  # estimated using the noncentred parameterisation for improved
  # efficiency.
  trend_formula = ~ s(season, bs = "cc", k = 6) + AR(),
  noncentred = TRUE,
...
Both series 1 and 2 share the exact same latent process estimates, while the estimates for series 3 are different (this is encoded in `trend_map`; see `summary(full_mod)` for the underlying `ar1_trend` and `sigma_trend` posteriors). Forecasts for series 1 and 2 will still differ because they have different intercepts in the observation model.
...
mod <- mvgam(
  ...
  trend_formula =
  # formula for the latent signal: depends nonlinearly on
  # productivity, with AR(1) temporal autocorrelation on top
    ~ s(productivity, k = 8) - 1 + AR(),
  noncentred = TRUE,
  trend_map =
...
A final but very key question is whether we can successfully recover the true hidden signal. The fit object exposes the latent process as the `trend[t, series]` parameter matrix; series 1 (any one of the three sensors works, since all three share the same latent state under the `trend_map`) gives the posterior mean signal at each time point. We can overlay the simulated truth to confirm recovery:
```{r}
trend_draws <- as.matrix(
  as_draws_df(mod, variable = "^trend\\[", regex = TRUE)
)
n_time <- length(true_signal)
trend_mean_series1 <- colMeans(trend_draws)[seq_len(n_time)]
ggplot2::ggplot() +
  ggplot2::geom_line(
    data = data.frame(time = seq_len(n_time),
                       y = trend_mean_series1),
    mapping = ggplot2::aes(x = time, y = y),
    colour = "#B23A48"
  ) +
  ggplot2::geom_point(
    data = data.frame(time = seq_len(n_time), y = true_signal),
    mapping = ggplot2::aes(x = time, y = y)
  ) +
