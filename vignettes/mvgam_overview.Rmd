...
All options are the same as for Random Walks, but additional options will be available for placing priors on the autoregressive coefficients. By default, these coefficients will not be forced into stationarity, but users can impose this restriction by changing the upper and lower bounds on their priors. See `?default_prior.mvgam_formula` for more details.
...
For setting up State-Space models, the optional process model formula can be used. See `?mvgam` and `?trend_formula` for guidance on using trend formulae.
...
```{r}
default_prior(
  mvgam_formula(count ~ s(year_fac, bs = "re") - 1),
  data = model_data,
  family = poisson()
)
```

See `?default_prior.mvgam_formula` for different ways that priors can be altered.
